...
def test_healthy_dataset_becomes_metric_observed_events() -> None:
    _, _, _, events = build_bank_01_events()

    assert len(events) == 36

    assert all(
        event.event_type == "metric.observed"
        for event in events
    )
...
def test_healthy_events_have_contiguous_sequences() -> None:
    _, _, _, events = build_bank_01_events()
...
def test_healthy_events_are_emitted_chronologically() -> None:
    _, _, _, events = build_bank_01_events()
    ...
    assert all(
        current <= following
        for current, following in pairwise(timestamps)
    )


def test_healthy_events_preserve_run_identity() -> None:
    _, context, _, events = build_bank_01_events()
    ...
    assert all(
        event.chg_id == context.chg_id
        for event in events
    )

    assert all(
        event.business_stream == context.business_stream
        for event in events
    )
...
def test_healthy_events_include_healthy_historical_context() -> None:
    _, _, _, events = build_bank_01_events()

    historical = events[0].data["metric"]["historical"]

    assert historical["perturbation_strength"] == 0.0
    assert historical["perturbation_phase"] is None
    assert "counterfactual_value" in historical


def test_each_healthy_timestamp_contains_all_metrics() -> None:
    _, _, _, events = build_bank_01_events()
...
def test_iter_historical_healthy_metric_events_is_lazy() -> None:
    scenario = load_scenario("config/scenarios/banking/BANK-01.yaml")
    enterprise = load_enterprise_configuration("config/enterprises/bank_alpha")
    runtime = build_historical_scenario_runtime(
        scenario=scenario,
        enterprise=enterprise,
    ...
    anchor_time = datetime(2026, 8, 14, 10, 0, tzinfo=UTC)
    dataset = build_historical_healthy_change_dataset(
        runtime=runtime,
        anchor_time=anchor_time,
        post_change_samples=6,
        random_source=SimulationRandom(seed=42),
    ...
    iterator = iter_historical_healthy_metric_events(
        dataset=dataset,
        runtime=runtime,
        context=context,
        ids=ids,
...
def test_healthy_events_do_not_emit_change_boundary_timestamp() -> None:
    (
        dataset,
        _,
        _,
        events,
    ) = build_bank_01_events()

    assert all(
        event.event_time != dataset.change_boundary_time
        for event in events
    )
